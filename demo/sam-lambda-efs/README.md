...
```bash
sam-lambda-efs$ sam logs -n HelloEfsFunction --stack-name "sam-lambda-efs" --tail
```
...
```bash
sam-lambda-efs$ pip install -r tests/requirements.txt --user
# unit test
sam-lambda-efs$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
sam-lambda-efs$ AWS_SAM_STACK_NAME="sam-lambda-efs" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "sam-lambda-efs"
```
