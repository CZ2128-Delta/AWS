...
def lambda_handler(event, context):
    ...
    # First update on info.json in user's directory
    ...
    info_dict[proj_name]['progress'] = str(0)

    json_str = json.dumps(info_dict, indent=4)
    bytes_data = json_str.encode('utf-8')
    s3_client.put_object(Bucket=bucket, Key=str(info_json), Body=bytes_data)

    # Create EFS subroot path
    ...
    print(subroot_path)
    # Download mrc to EFS ( mounted on an EC2 instance )
    ...
    for i, frame in enumerate(mrc_data):
        ...
        # Downsample images if too large
        ...
        if W > MAX_IMAGE_SIZE or H > MAX_IMAGE_SIZE:
            if W > H:
                ...
                new_W, new_H = int(W * MAX_IMAGE_SIZE / H), MAX_IMAGE_SIZE
            frame = frame.resize((new_W, new_H), resample=Image.Resampling.LANCZOS)
        # Save images
        ...
        # Generate json for each png
        png_dict = {}
        png_dict['bucket_name'] = bucket
        png_dict['user_name'] = user_root_path.stem
        png_dict['project_name'] = proj_name
        png_dict['file_name'] = f'{mrc_name.stem}_{str(i).zfill(5)}.png'
        png_dict['encoded_png_data'] = base64_string
        json_bucket_key = f'{mrc_name.parent}/{mrc_name.stem}/{mrc_name.stem}_{str(i).zfill(5)}.json'
        s3_client.put_object(Body=json.dumps(png_dict, indent=4), Bucket=bucket, Key=json_bucket_key)
    ...
    # Second update on info.json in user's directory
    response = s3_client.get_object(Bucket=bucket, Key=str(info_json))
    content = response['Body'].read().decode('utf-8')
    info_dict = json.loads(content)
    ...
    info_dict[proj_name]['progress'] = str(1)
